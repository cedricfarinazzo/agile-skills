...
- **Flake-vs-regression diagnosis (before rerunning).** When CI fails on a test that is NOT in the PR diff, the instinct is "flake — rerun." This is wrong as often as it is right. Before `gh run rerun --failed`, do a 60-second diagnosis:
  ...
  4. **REACHABILITY beats repeat count.** Before concluding "identical rerun ⇒ real failure", ask whether the diff can even reach the failing test. If the source subtree the test exercises is **byte-identical to a base branch on which that same test is green**, the diff is not the cause however many times the failure repeats — classify it environmental and look at load/timing. Observed in a long autonomous run: a test failed identically twice on an untouched subtree while a concurrently-running sibling PR passed the identical test on the identical tree in the same window; the failing PR passed on the first attempt once the shared runner was freed. Only when the diff *can* reach the test does an identical repeat mean a real failure — then root-cause it as a Critical PR issue.
