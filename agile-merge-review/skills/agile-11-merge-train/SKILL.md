...
| `3b merge-review-pr` | **Reviewed sha**; **Files-read list** (= diff set); a **cite per lens** (including the **invariants/conventions** lens); **per-AC line binding**; verdict | compute `gh pr diff <N> --name-only`; **reject if Files-read ≠ diff set**, reject any AC with no line cite, reject a bare pass; **record the reviewed sha — 3f gates on it** |
...
- **Review against the documented invariants/conventions too, not only the ACs and the out-of-scope section.** A change that makes a documented invariant FALSE is a defect even when the code is correct (e.g. widening the effective key of a documented invariant without updating the doc). Two corollaries the receipt must cover: a "KNOWN LIMITATION" comment that outlives its limitation is its own defect — the ticket that removes the limitation removes or rewrites the comment; and a documentation entry is verified **accurate against the shipped code**, not merely verified present, since a confidently-worded wrong entry is worse than none. The receipt carries an **invariants/conventions** lens cite (or an explicit "N/A none touched") like every other lens.
...
- **The `integration-deferred` label (Phase 0) confirms where integration lives.** `merge-fix-until-satisfied` runs lint locally and relies on **3e's** fresh CI-green run for the test tiers — the same model for every PR. The label just makes explicit that for a concurrent build the integration + e2e tiers were never run at build time, so **3e's fresh-CI-green hard gate is their definitive gate** — it is required before merge regardless, and doubly load-bearing here. Do not treat a missing local integration run on an `integration-deferred` PR as a defect; that is by design.
...
- Stale-run risk: a poll immediately after `git push` can read the *previous* run as `COMPLETED + SUCCESS`. The snippet above is the mitigation — capture the id before the push, wait until a different id appears. Do not substitute a head-following loop.
...
- **Flake-vs-regression diagnosis (before rerunning).** When CI fails on a test that is NOT in the PR diff, the instinct is "flake — rerun." This is wrong as often as it is right. Before `gh run rerun --failed`, do a 60-second diagnosis:
  ...
  3. Only after both checks pass should you call it a flake and rerun.
  4. **REACHABILITY beats repeat count.** Before concluding "identical rerun ⇒ real failure", ask whether the diff can even reach the failing test. If the source subtree the test exercises is **byte-identical to a base branch on which that same test is green**, the diff is not the cause however many times the failure repeats — classify it environmental and look at load/timing. Only when the diff *can* reach the test does an identical repeat mean a real failure — then root-cause it as a Critical PR issue.
  5. **Corollary — retry a timing-sensitive failure on an UNCONTENDED runner.** On a capacity-limited shared runner, retrying while other heavy jobs are in flight reproduces the same timeout every time and reads as a real defect. Serialise the retry behind the other in-flight jobs rather than stacking reruns under contention.
