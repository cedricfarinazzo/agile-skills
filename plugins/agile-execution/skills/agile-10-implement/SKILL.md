...
On Claude Code, use the named `agile-execution:<agent>` definitions shown below and invoke sub-skills with the Skill tool. On Codex, use the matching name in this plugin's `.codex/agents/` directory and invoke the sub-skill as `$<skill-name>`. Both hosts preserve the same phase ordering, receipts, and gates. If subagents are unavailable, use the documented inline fallback.
...
| `implement` | each gate command **+ its real exit code**; AC→test coverage; **a `Mutation:` line naming the defect introduced and the RED count**; **a `repeat:` line** on any intermittent-failure fix; **a `seam:` line** on any diff that adds, renames or drops a field crossing a producer→consumer boundary; (concurrent) tier-deferral note | `git show --stat <branch>` confirms the pushed commits; **AC coverage with no `Mutation:` line = not-run** — a test that cannot fail is not coverage; a stated `DEFERRED TO CI` / `n/a — <reason>` counts, a silent omission does not. **A diff that fixes a race, flake or ordering bug with no `repeat:` count = not-run** — one green run is the outcome the bug produces anyway. **A boundary-crossing field change whose `seam:` line reddens only the producer's own test = not-run** — that red proves nothing; re-dispatch for a consumer-side red or a shared artifact |
...
- **`concurrency=N>1` — N caps the tickets IN FLIGHT; it is not a batch size.** Each ticket runs the SAME per-phase chain as `concurrency=1`, in its own named agent per phase, and advances that chain independently — siblings sitting at *different* phases at once (one planning, one implementing, one in a fix cycle) is the intended steady state, not a defect. A ticket leaving the pipeline frees its slot immediately. A worktree isolates the filesystem but not the stack, so a concurrent `implement` runs the **stack-free gate only** (lint + unit + typecheck + migration linearity) and defers the **stack-bound tiers** (integration, e2e, apply-on-fresh-DB) to CI — see `implement-code`.
  - **ONE worktree per TICKET, shared by that ticket's whole phase chain.** Every phase needs the code — `validate` and `plan` read it, `implement` writes it, `pr` reads the resulting diff — and they should all read the SAME tree. So the isolation boundary is the ticket, not the phase:
    ...
    1. **The orchestrator creates it**, before dispatching `validate`, at a deterministic path under `.agents/worktrees/` named for the ticket, on the ticket's feature branch:
       `git worktree add .agents/worktrees/<ticket-key> -b <branch-prefix><ticket-key>-<slug> <base-branch>`
  ...
  - **The scratch/temp directory is shared state too — the worktree only isolates the repo.** Concurrent agents inherit the SAME temp dir, so a generic filename (`pr.md`, `body.txt`, `notes.md`) is a cross-ticket collision: a sibling overwrites it between your write and your read, and you publish its ticket's content under yours. Name every scratch file for its work item (`pr-<ticket-key>.md`), and treat any write-then-use pair across a shared path as non-atomic — re-read what actually landed before reporting done.
...
The per-ticket worktrees this run created (`.agents/worktrees/<ticket-key>`) are the orchestrator's to remove — nothing auto-cleans them, because they are shared across a phase chain rather than owned by one agent. A lingering one still holds its branch, which makes a later branch delete fail *after* the merge already happened. Clean up at the source, in one pass after Phase 2:
