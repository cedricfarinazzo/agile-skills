...
On Claude Code, use the named `agile-merge-review:<agent>` definitions and invoke sub-skills with the Skill tool. On Codex, use the matching name in this plugin's `.codex/agents/` directory and invoke the sub-skill as `$<skill-name>`. Preserve the same sequential gates and receipts on both hosts; use the inline fallback when subagents are unavailable.
