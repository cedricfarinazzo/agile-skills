...
**Both orchestrators are invoked inline in this context — never wrapped in a subagent.** In Claude Code invoke them with the Skill tool; in Codex invoke `$agile-10-implement` and `$agile-11-merge-train`. An orchestrator is itself a dispatcher, so this layer has no dispatch mode of its own. The leaf phases still use their host's scoped subagents and return capped receipts.
