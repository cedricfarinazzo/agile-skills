...
- **Worktree cleanup** — a `concurrency>1` run ends by removing the worktrees whose branch has merged, keeping unmerged ones (open PR / salvage material) and never forcing past uncommitted work. Left alone they accumulate and keep holding their branch, turning a later branch delete into a spurious post-merge failure.
