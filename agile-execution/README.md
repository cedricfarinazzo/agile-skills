...
| `agile-execution:self-reviewer` | `implement-review` | sonnet / high — validates and publishes the author's six-lens verdict |
| `agile-execution:review-lens` | `implement-review` lens fan-out (opt-in, large PR only) | sonnet / high — supplies read-only evidence that `self-reviewer` validates and publishes |
...
- **Dispatch-and-verify** — the orchestrator runs no step's work itself. Every phase, including `implement-review`, runs in its named `agile-execution:*` agent and returns a **receipt**; the orchestrator verifies it against ground truth (git / `gh` / Jira) before advancing. For a large PR it first fans out read-only `review-lens` evidence, then `self-reviewer` validates, aggregates, and publishes the verdict. A validate `pass` with no score breakdown, a review whose files-read list ≠ the PR diff, or a phase whose receipt is contradicted is re-dispatched — this is what stops silent shortcuts (skimmed review, skipped gate).
