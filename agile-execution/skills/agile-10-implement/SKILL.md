...
| `implement` | each gate command **+ its real exit code**; AC→test coverage; **a `Mutation:` line naming the defect introduced and the RED count**; (concurrent) tier-deferral note | `git show --stat <branch>` confirms the pushed commits; **AC coverage with no `Mutation:` line = not-run** — a test that cannot fail is not coverage; a stated `DEFERRED TO CI` / `n/a — <reason>` counts, a silent omission does not |
