...
**The main agent orchestrates; it never does a step's work in its own context.** Every phase runs in its named `agile-execution:*` subagent (table above; inline via the Skill tool instead, only under `concurrency=0`) that executes the sub-skill and returns **only a structured receipt — proof fields, nothing else** (the fields below; no prose sections, no narrative, no transcript). The orchestrator's loop is: **dispatch the step-subagent → read its receipt → verify the receipt against ground truth (git / `gh` / Jira) → gate advancement.** A step whose receipt is missing, incomplete, or contradicted by ground truth is **not done** — re-dispatch it; never wave it through on the subagent's word alone.
