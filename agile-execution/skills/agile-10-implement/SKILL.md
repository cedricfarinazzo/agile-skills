...
5. **`implement-review`** (`agile-execution:self-reviewer`) → all six lenses from one read, verdict posted to PR + Story. For a large PR, first fan the lens groups out as parallel `review-lens` subagents yourself, then dispatch `self-reviewer` with their receipts to validate, aggregate, and publish the single verdict.
