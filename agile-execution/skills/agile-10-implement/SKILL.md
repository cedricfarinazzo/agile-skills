...
**The main agent orchestrates; it never does a step's work in its own context.** Every phase runs in its named `agile-execution:*` subagent (table above; inline via the Skill tool instead, only under `concurrency=0`) that executes the sub-skill and returns **only its receipt** (the fields below). No receipt may carry a preamble, an overview/summary section, or a "what was good"/praise section — the orchestrator pays for those in context and verifies nothing with them. A **mechanical** phase (`validate`, `plan`, `implement`, `pr`, `monitor`) returns **structured fields only** — no narrative, no transcript. The **review** phase (and each `:review-lens` slice) returns its proof fields **plus its findings**, and prose *within* an individual finding or a per-AC binding is expected: a finding flattened to a label cannot be acted on. The orchestrator's loop is: **dispatch the step-subagent → read its receipt → verify the receipt against ground truth (git / `gh` / Jira) → gate advancement.** A step whose receipt is missing, incomplete, or contradicted by ground truth is **not done** — re-dispatch it; never wave it through on the subagent's word alone.
...
**Dispatching a WHOLE ticket? Name the post-code phases, or the agent stops at push.** `implement-code` correctly finishes at commit+push and does **not** open the PR — that boundary is right per-phase, but when you hand ONE agent a whole ticket expecting a finished PR, that same boundary makes it stop after push with the PR unopened and the ticket never moved to In-Review. It emits a valid receipt, just for under-scoped work. So a single-agent whole-ticket dispatch MUST enumerate the `pr` (open PR) + `status_change` (transition to In-Review) steps in its prompt; otherwise the orchestrator MUST run those phases itself after the bare code dispatch. Make the boundary explicit so a lone build dispatch never silently under-delivers.

## Three-way concurrency split: inline / sequential-dispatch / concurrent-dispatch

**`concurrency=0`: fully inline, no agent dispatch, no worktree.** Every phase runs directly in the orchestrator's own context via the Skill tool — no `agile-execution:*` subagent, no `Agent` tool call at all, at any layer. Same per-ticket sequence, same resume/reconcile logic, same receipt requirements (the orchestrator still writes and checks its own receipt before advancing) — only the dispatch hop is removed. Use this wherever subagent/worktree dispatch isn't available or wanted — most importantly when this skill is **itself** invoked from a dispatched context (dispatch nesting depth is 1, so no further agent can be spawned), and when stepping through a single ticket by hand. Opt-in, and the sanctioned fallback (Phase 0.5); the default is `1`.
...
**A concurrent agent git-mutates ONLY inside its own worktree — never the shared base checkout.** Under concurrency a build/review/fix agent that runs a tree-wide `checkout`, switches branches, or commits in the shared repo corrupts a sibling's state — a stray commit on the base branch, a commit landing on the wrong branch, a clobbered untracked file. To read another ref use `git show <sha>:<path>`, never a tree-wide `checkout`; do ALL mutation in your dedicated worktree; if no worktree is available, do NOT git-mutate — emit the receipt with a `blocked` field.
...
**Graceful fallback — inline is sanctioned when dispatch is unavailable.** Step down one rung at a time and say which rung you landed on:
1. worktree subagents can't be created → `concurrency=1` (sequential dispatch, no worktree);
2. **agent dispatch itself is unavailable** (this skill is already running inside a dispatched context — dispatch nesting depth is 1 — or the tooling has no agent) → **`concurrency=0`, fully inline**. That is exactly the mode `concurrency=0` names, and it is the *correct* answer here, not a violation: the alternative is not running at all.

**Receipt discipline is mandatory in every mode — inline most of all.** Inline is the weaker mode: a phase run in the orchestrator's own context can be skimmed, skipped, or rubber-stamped across a long run, and its detail bloats the loop. So inline does not relax the gate — write each phase's receipt out explicitly and verify it against ground truth (git / `gh` / Jira) before advancing, exactly as if a subagent had returned it. "I already did that step" is not a receipt.
...
- **Dispatch every phase to its named agent and verify its receipt; run inline only under `concurrency=0` — and never without the receipt.** Each of `implement-validate` / `implement-plan` / `implement-code` / `implement-pr` / `implement-review` / `implement-monitor` runs in its named `agile-execution:*` subagent (table above) that returns its receipt — structured fields only for the mechanical phases, fields plus findings for the review phase (see above); the orchestrator verifies that receipt against ground truth (per the receipt-verification table) before advancing. Under `concurrency=0` the orchestrator runs each sub-skill inline via the Skill tool instead — same receipts, same verification, no agent hop. The orchestrator owns selection, ordering, the per-ticket sequence, and the report — it reads no changed files, writes no review, and scores no ticket itself (dispatch mode) or does so transparently in its own context (inline mode). A phase whose receipt is missing/incomplete/contradicted is re-dispatched, not waved through. Fix a sub-skill in its own file, never fork its logic here.
...
- **A wrong ticket is corrected in the open, not rejected and not obeyed literally.** When an AC names a file, test, or symbol that does not exist — or one that pins something other than what the AC describes — the ticket is not under-specified and the literal text is not authoritative. `implement-plan` establishes ground truth, posts a `🤖 <!-- agile:spec-correction -->` comment with evidence, and satisfies the AC **by intent**; the PR body repeats it so the reviewer meets the correction before the diff. Reject only when the *intent* is unrecoverable. Never edit the ticket's AC text, and never leave the correction living only in your context — an unposted correction reads downstream as an unexplained deviation.
...
- **Clean up the worktrees you created (Phase 2b).** Remove merged ones, keep unmerged ones, never `--force`, report both. Best-effort.
