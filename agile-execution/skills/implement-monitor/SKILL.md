...
2. **Failing status checks.** Poll `statusCheckRollup`. On `FAILURE` / `UNSTABLE`, run the **flake-vs-regression diagnosis** before re-running — never blind-rerun. The diagnosis, in order:
   ...
   - **Triage by severity, not by volume.** A gate prints warnings and infos around the one error that failed it — plus, often, a "N diagnostics not shown" truncation notice. Re-run the tool restricted to error level (or otherwise split errors from warnings) and diagnose the failing item, never the noisiest one.
   - **A SKIPPED job is a symptom.** A downstream job whose gate failed reports SKIPPED, which reads as "did not run" rather than "broke" — a red PR then looks merely incomplete. Walk the job-dependency chain back to the gate that actually failed before diagnosing.
   - **"Pre-existing" / "unrelated" / "environment" / "tooling drift" needs base-branch proof, not a reading of the output.** Run the SAME command on the base branch and compare exit codes (and diff the tool's config/lockfiles between the two): clean on base + non-zero on the branch = the diff caused it. **Filenames in the output being untouched by your diff is not evidence** — a diff can cause a failure reported against files it never edited. Without the comparison, do not report the check as pre-existing.
   ...
   - **REACHABILITY beats repeat count.** An identical repeat failure does not prove a real failure. Ask first whether the diff can reach the failing test: if the source subtree that test exercises is **byte-identical to a base branch on which the test is green**, the diff cannot be the cause however often it repeats — classify it environmental and look at load/timing (a sibling PR passing the identical test on the identical tree in the same window is the tell). **Retry a timing-sensitive failure UNCONTENDED** — serialise it behind the other in-flight heavy jobs; repeated retries during contention produce repeated identical failures and read as a real defect.
