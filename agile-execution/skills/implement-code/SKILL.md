...
**Build mode** — the orchestrator passes `mode=sequential | concurrent` in the dispatch prompt (default `sequential`):
...
- **A test's expected value comes from the system under test or an invariant — never a hand-guessed constant.** A number you reasoned out by hand can encode a wrong premise that passes locally and only fails at the integration/stack tier. Assert a relationship/invariant, or capture the value FROM the system under test as a golden; never assert a magic number you guessed.
...
- **For a change with a runtime surface, verify it by exercising the flow — not just green tests.** Green unit + integration proves the covered code behaves; it does not prove the feature *works*. Drive the actual path the change affects (run the app / hit the endpoint / render the view) and observe the real result before finishing. Bugs that pass every test but fail in use — a stale cache after a mutation, a missing affordance, an unwired handler — are caught here, not in the suite. **Concurrent mode:** if exercising the flow needs the shared stack (running the app, hitting an endpoint, a DB), do **not** run it in the worktree — it would collide with sibling builds on the same ports/DB. Defer that runtime check to the serial Phase 2 (`implement-monitor`, which holds the stack) / CI; a purely stack-free check (rendering a component, a CLI invocation) may still run in-worktree.
...
2. **Stack-free tests** pass green — **unit** (+ typecheck), run locally, no skips/xfails hiding a failure. *(both modes)* **Caveat:** if this project's "unit" tests actually hit the DB / need the stack, they are **not** stack-free — the run must be `concurrency=1` (say so and stop rather than running them in a worktree).
...
**Checkpoint-commit early so a mid-build death doesn't strand the work (the gate still governs hand-off).** The rule above — commit + hand off only when every gate is green — is about *finishing*, not about *never committing until then*. This phase can die mid-flight for reasons unrelated to the code: a **session/usage limit, an API error, a runner OOM, a crash**. If no commit exists yet, everything is stranded in the working tree — and in concurrent mode (a build-subagent per ticket in its own worktree) a re-dispatch spawns a **fresh** worktree off base, so that uncommitted work is lost and the ticket restarts from scratch. Avoid it: **once the code compiles and the branch exists, commit a WIP checkpoint and `git push` it** (`wip: <TICKET> …`, or amend as you go), then keep working toward the gate. Pushing early costs nothing — the PR isn't opened until `implement-pr`, and the orchestrator only counts the ticket handed off on the *gated* `implement` marker — but a death after the code is written then leaves recoverable commits on the remote branch instead of an orphaned worktree. Before the final hand-off, squash/amend the WIP into the single intended conventional commit. Do **not** post the `🤖 implement` marker or hand off on a WIP push — the gate is unchanged; only the *finished* commit is green-gated.
