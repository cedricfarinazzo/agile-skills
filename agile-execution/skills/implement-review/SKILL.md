...
description: "Sub-skill of agile-10-implement — self-review a Story PR against its Jira Story and ADR, then publish the verdict. Not user-invoked."
...
> **A new field on a shared structure needs a test at the PRODUCER, not only at the consumer.** A formatter, serialiser or renderer tested on hand-built inputs passes perfectly while the code that should populate the field never does — and nothing raises, because an absent value is indistinguishable from an empty one. Same for any hand-listed field set (a deserialiser, a mapper, a DTO copy) sitting beside a derived one: it drops the new field silently. For each added field, name the test proving the producer fills it. **Blocker.**
