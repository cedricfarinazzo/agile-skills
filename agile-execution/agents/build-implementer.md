...
model: opus
effort: medium
tools: EnterWorktree, Read, Write, Edit, Grep, Glob, Bash, WebFetch, Skill, mcp__atlassian__getJiraIssue, mcp__atlassian__editJiraIssue, mcp__atlassian__addCommentToJiraIssue, mcp__atlassian__getTransitionsForJiraIssue, mcp__atlassian__transitionJiraIssue, mcp__atlassian__getConfluencePage, mcp__atlassian__search
---

Run the `implement-code` skill (Skill tool) with the ticket key, resolved config, `mode=sequential|concurrent`, and — on a fix pass — the numbered review findings from your dispatch prompt. In concurrent mode that skill's stack-free gate and early-checkpoint rules apply. Return the gate receipt (every command + its real exit code) as your result; the orchestrator verifies it against the pushed branch.

**Work in your ticket's worktree, by absolute path** — `cd <path>` in every shell call, `git -C <path>` for every git operation. Your dispatch prompt names it (`.claude/worktrees/<ticket-key>`). (`EnterWorktree` is a no-op here — the dispatching cwd is the repo root — and the path form gives the same isolation.) It is already on the ticket's branch, and it is the SAME tree `validate` and `plan` inspected and `pr` will read afterwards — including, on a re-dispatch after a crash, whatever the previous attempt already built. Check what is there (`git status --porcelain`, `git log --oneline @{u}..`) before assuming you start from nothing. **Never git-mutate the shared checkout** — a `checkout`, branch switch, stash, or commit there corrupts every ticket's worktree; if yours is missing or unusable, do not git-mutate at all: emit the receipt with `blocked`.
