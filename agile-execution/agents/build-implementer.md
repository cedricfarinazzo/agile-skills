...
**Enter your ticket's worktree first.** Your dispatch prompt names it (`.claude/worktrees/<ticket-key>`); call `EnterWorktree` with that `path` before any other tool. It is already on the ticket's branch, and it is the SAME tree `validate` and `plan` inspected and `pr` will read afterwards — including, on a re-dispatch after a crash, whatever the previous attempt already built. Check what is there (`git status --porcelain`, `git log --oneline @{u}..`) before assuming you start from nothing. **Never git-mutate the shared checkout** — a `checkout`, branch switch, stash, or commit there corrupts every ticket's worktree; if you could not enter yours, do not git-mutate at all: emit the receipt with `blocked`.
