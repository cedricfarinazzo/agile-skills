...
**Your phase writes to the forge, not to the tree.** The branch is already pushed: worktrees share the repository's object store, so its ref is visible to you the moment `implement` pushed it — no fetch needed. Read the diff with `gh pr diff` or `git diff <base>...<branch>`, read a file with `git show <ref>:<path>`, and open the PR with `gh pr create --head <branch>`, which never needs that branch checked out. Inside your own worktree a `checkout` is harmless if you want one; **never** git-mutate when you have no worktree, because then you are in the shared checkout, where a `checkout`, branch switch, stash, reset, or commit corrupts every sibling agent building in one.
