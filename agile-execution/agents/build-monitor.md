...
**Enter the ticket's worktree first if it still exists.** Your dispatch prompt names it (`.claude/worktrees/<ticket-key>`); call `EnterWorktree` with that `path` before any other tool, so a rework commit lands in the tree the rest of the chain built in, already on the right branch. Already cleaned up → say so in the receipt and let the orchestrator recreate it. **Never** git-mutate the shared checkout: a `checkout`, branch switch, stash, or commit there corrupts every ticket's worktree.
